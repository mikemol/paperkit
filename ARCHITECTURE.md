...
| `resolver.py` | Check-RESOLUTION core: **owns `VERBS`**, the one declaration of the built-in verb set (every doc/witness derives from it) + one dispatch branch each, plus custom types; `_library_for` resolves `concept:` to the **consuming project's** library (`<project>/library/` or `<repo>/library/`) and **falls back to the engine's** — the fallback is silent, so a project whose library is missing or differently named gets the engine's and sees "unknown concept key" listing keys it never wrote (the error names the library path it used, so the message points at the resolution rather than at the bib); env sanitization (`clean_env`, sshd-style default-deny + PATH pin), footprint tracing via strace. | stdlib, `config`; subprocess→`gate.py` (`result:`), `library/concepts.py` (`concept:`) | gate, grader, footdeps, many tests, paper checks |
...
| `checks/readme.py` | Per-claim discriminating witnesses for README (`claim:` type); each asserts its claim against real engine behavior → Δ behavioral. | stdlib, engine `gate/project/grade`; reads engine source + assets | `paper.toml [checks.claim]` (via gate/discriminate) |
...
`ENGINE_SRCS`. Many share the `_fixture_*` capability modules.

- `_fixture_model.py` / `_fixture_project.py` / `_fixture_gate.py` / `_fixture_delta.py` —
  the validated minimal-project builder, split per capability (Μ·kernel·fixture·split):
  the model kernel (`entry/_write/_call`, no engine import) plus one module per engine
  capability (`project_text`, `gate/gate_json`, `discriminate/discriminate_stderr`), each
  importing its engine module at module top so `dag.bzl` carries the honest per-capability
  cone and a witness's closure collapses to the subsystem it exercises.
