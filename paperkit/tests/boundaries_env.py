...
injection vectors (LD_PRELOAD, IFS, BASH_ENV, PYTHONPATH) and any unknown var.
...
DIRTY = {
    "PATH": "/usr/bin", "HOME": "/h", "LC_ALL": "C",                 # kept: needed
...
def main() -> int:
    ...
    check("keeps PATH so a check's tools still resolve", e.get("PATH") == "/usr/bin")
