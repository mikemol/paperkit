...
[^rnd-deck-bound]: The bound is ABOUT the coalgebra's slide nodes — it grounds on the graph, not on the odf node that precedes it in prose (grounded above in [@rnd-graph])
[^rnd-slides]: Walks the morphisms the graph declares, under the bound the previous claim states
[^rnd-units]: The observed route's own drift gate, beside the deck it sources
